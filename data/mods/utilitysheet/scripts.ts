	...
	init() {
		...
		const movepoolSections = {
			...
			Disrupt: [
				...
				'rashpowder', 'sacredfire', 'saltcure', 'scald', 'scorchingsands', 'sleeppowder', 'spore', 'stunspore', 'thunderwave', 'toxic', 'toxicthread', 'willowisp',
				// Setup Control
		...
		for (const id in this.dataCache.Pokedex) {
			...
			if (this.dataCache.Learnsets[id] && this.dataCache.Learnsets[id].learnset) {
				...
				let sheetOutput: string[] = [
					(poke3 ? poke.kind + `~` + (printno) + `~1~=IMAGE("https://github.com/Scoopapa/DH/blob/master/data/mods/g9blindsided/sprites/icons/` + poke3id + `.png?raw=true",3)~~` + poke3.name + "~" + poke3.types[0] + "~" + (poke3.types[1] ? poke3.types[1] : "") + "~" + poke3abilities + "~" + poke3.baseStats.hp + "~" + poke3.baseStats.atk + "~" + poke3.baseStats.def + "~" + poke3.baseStats.spa + "~" + poke3.baseStats.spd + "~" + poke3.baseStats.spe + "~" + `\n` : "")
					+ (poke2 ? poke.kind + `~` + (printno) + `~2~=IMAGE("https://github.com/Scoopapa/DH/blob/master/data/mods/g9blindsided/sprites/icons/` + poke2id + `.png?raw=true",3)~~` + poke2.name + "~" + poke2.types[0] + "~" + (poke2.types[1] ? poke2.types[1] : "") + "~" + poke2abilities + "~" + poke2.baseStats.hp + "~" + poke2.baseStats.atk + "~" + poke2.baseStats.def + "~" + poke2.baseStats.spa + "~" + poke2.baseStats.spd + "~" + poke2.baseStats.spe + "~" + `\n` : "")
					+ poke.kind + `~` + (printno) + `~3~=IMAGE("https://github.com/Scoopapa/DH/blob/master/data/mods/g9blindsided/sprites/icons/` + iconid + `.png?raw=true",3)~~` + poke.species + "~" + poke.types[0] + "~" + (poke.types[1] ? poke.types[1] : "") + "~" + abilities + "~" + poke.baseStats.hp + "~" + poke.baseStats.atk + "~" + poke.baseStats.def + "~" + poke.baseStats.spa + "~" + poke.baseStats.spd + "~" + poke.baseStats.spe + "~" + `\n`
					+ poke.kind + `~` + (printno) + `~4~\n`
