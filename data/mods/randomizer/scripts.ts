				...
				for (const level in poke.learnsetCumulative.learnset) {
					if (parseInt(level) > 99) {
						if (poke.recommendedLvUp.length) {
							...
							sheetOutput += `\n=row()-1~13~~Additional level-up candidates (pick up to ` + (22 - levelUpMovesCount + poke.extraLevelUpSpaces) + `)\n=row()-1~14~`
						...
						if (poke.forcedMoves.length) {
							...
							sheetOutput += `\n=row()-1~15~~Possible synergistic moves\n=row()-1~16~`
					...
					if (poke.learnsetCumulative.learnset[level].movesLearned.length) {
						...
						if (parseInt(level) > 99) {
							...
							for (const moveid of poke.learnsetCumulative.learnset[level].movesLearned) {
								sheetOutput += `\n=row()-1~19~` + (poke.prevo ? ` ` : `~`) + ((poke.prevo && this.dataCache.Pokedex[this.toID(poke.prevo)].prevo) ? ` ` : `~`) + (parseInt(level) + 1) + ` - ` + moveid;
								levelUpMovesCount++;
							}
						}
					}
